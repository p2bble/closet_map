...
| 🔄 계절 허브 | 전환 진행률 카드 + 보관/꺼내기 + 보관 전 체크리스트(세탁·상태·방충제) + 지난 보관 기록 — "그 패딩 어디 넣었더라"의 답이 탭 첫 화면에 |
| ⚠️ 미세탁 경고 | 세탁하지 않고 보관한 옷을 꺼낼 때 경고 메시지 표시 (개별·일괄 모두 지원) |
| 🧺 세탁 필요 뱃지 | 3회 이상 착용한 옷에 세탁 필요 뱃지 표시 — 보관 전 세탁 자연스럽게 유도 |
| ⚠️ 방치 옷 알림 | 현재 계절 옷 중 6개월 이상 꺼내지 않은 옷을 홈 브리핑 + 로컬 알림으로 안내 |
| 💾 백업/복원 | 옷·보관 장소·사진·코디 기록 전체를 파일로 내보내기/가져오기 |
...
| 🏠 홈 브리핑 | 계절 인사 헤더(시즌별 컬러 테마) + 세탁/방치/보관대기 브리핑 리스트 + 이번 계절 옷 셸프 |
...
└── screens/
    ...
    └── season_tab.dart          # 계절 허브 (진행률·체크리스트·보관 기록) + 보관/꺼내기 화면
```
...
| 버전 | 내용 |
|---|---|
| v1.9.0 | UI/UX 리뉴얼 — 홈 B안 재구성(계절 인사 헤더·세탁/방치/보관대기 브리핑 리스트·이번 계절 옷 셸프), 계절 허브(전환 진행률·상시 체크리스트·지난 보관 기록), 시즌 컬러 시스템(계절별 소프트 틴트), 코디 삭제 되돌리기 스낵바, 빈 상태 CTA, 공유 라벨 개선("디매"→"커뮤니티 양식") |
| v1.8.0 | 백업/복원 — 옷·보관 장소·사진·코디 기록 전체를 파일로 내보내기/가져오기 |
| v1.7.2~3 | 사진 영구 저장(image_store)·알림 권한·데이터 정합성·동기화 개선 |
...
| v1.5.0 | 코디 기록 시트 상의·하의·신발·악세 탭 분리, 커뮤니티 양식 텍스트 자동 생성 및 클립보드 복사, 저장 직후 공유 유도 스낵바 |
...
> 최신 버전: v1.9.0 — UI/UX 리뉴얼 (홈 브리핑·계절 허브·시즌 컬러 시스템)
